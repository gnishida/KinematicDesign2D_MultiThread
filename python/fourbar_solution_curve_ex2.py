...
T_list = []
T1 = np.array([[0.37955, -0.92516, 4.2], [0.92516, 0.37955, 1.2]])
T_list.append(T1)
T2 = np.array([[0.94385, -0.33035, 1.52], [0.33035, 0.94385, 1.02]])
T_list.append(T2)
T3 = np.array([[0.667119, 0.74495, 0.6], [-0.74495, 0.667119, 3.52]])
T_list.append(T3)
T4 = np.array([[0.97648, 0.21558, 0.63], [-0.21558, 0.97648, 1.3]])
T_list.append(T4)
...
file = open('solution_curve_ex2.txt', 'w')
	...
	for wy_index in range(wy_num + 1):
		...
		for u_index in range(u_num + 1):
			...
			for v_index in range(v_num + 1):
				...
				W.append([w[0], w[1]])
				G.append([x[2], x[3]])
				file.write(str(w[0]) + "," + str(w[1]) + "," + str(x[2]) + "," + str(x[3]) + ",OK\n")
