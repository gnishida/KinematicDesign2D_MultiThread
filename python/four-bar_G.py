...
data = np.zeros((v_num + 1, u_num + 1), dtype='f')
...
for u_index in range(u_num + 1):
	...
	for v_index in range(v_num + 1):
		...
		data[v_index][u_index] = min_f
		
data = np.flipud(data)
#print data

plt.imshow(data, interpolation='nearest', cmap="hot")
plt.xticks(np.arange(0, u_num + 1, resolution), np.arange(u_min, u_max + 1, 1))
plt.yticks(np.arange(v_num, -1, -resolution), np.arange(v_min, v_max + 1, 1))
plt.show()
