	...
	void LinkageSynthesisRRRP::calculateSolution(const std::vector<glm::dmat3x3>& poses, const std::vector<glm::dvec2>& linkage_region_pts, const std::vector<glm::dvec2>& linkage_avoidance_pts, int num_samples, const Object25D& moving_body, std::vector<Solution>& solutions, std::vector<glm::dvec2>& enlarged_linkage_region_pts) {
		...
		for (int scale = 1; scale <= 3 && solutions.size() == 0; scale++) {
			...
			for (int i = 0; i < threads.size(); i++) {
				threads[i] = boost::thread(&LinkageSynthesisRRRP::calculateSolutionThread, this, boost::ref(poses), boost::ref(enlarged_linkage_region_pts), boost::ref(enlarged_bbox), boost::ref(linkage_avoidance_pts), num_samples / NUM_THREADS, boost::ref(moving_body), boost::ref(sub_solutions[i]));
			}
			for (int i = 0; i < threads.size(); i++) {
				threads[i].join();
			}
