		...
		for u_index in range(u_num + 1):
			...
			for v_index in range(v_num + 1):
				...
				if orderCheck(T_list, x[0], x[1], x[2], x[3]):
					W.append([x[0], x[1]])
					G.append([x[2], x[3]])
					file.write(str(x[0]) + "," + str(x[1]) + "," + str(x[2]) + "," + str(x[3]) + ",OK\n")
				else:
					W2.append([x[0], x[1]])
					G2.append([x[2], x[3]])
					file.write(str(x[0]) + "," + str(x[1]) + "," + str(x[2]) + "," + str(x[3]) + ",NG\n")
...
if G.shape[0] > 0:
	plt.plot(G[:,0], G[:,1], 'b.')
if W2.shape[0] > 0:
	plt.plot(W2[:,0], W2[:,1], 'r.', alpha=0.5)
if G2.shape[0] > 0:
	plt.plot(G2[:,0], G2[:,1], 'b.', alpha=0.5)
plt.show()
